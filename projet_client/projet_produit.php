   ...
   <div id="sidebar-wrapper" data-simplebar="" data-simplebar-auto-hide="true">
     <div class="brand-logo">
      <a href="index.html">
       ...
       <h5 class="logo-text">GCS PROJET</h5>
     </a>
   </div>
...
<div class="col-4">
  ...
  <div class="col-4 offset-4">
    <a class="btn btn-light" href="utilisateur-create.php">Ajouter un nouvel Utilisateur</a>
  </div>
</div>
    ...
    <table class="table align-items-center table-flush table-borderless">
      <thead>
        <tr>
          ...
          <th width="25%">Type utilisateur</th>
...
<?php 
                                        while ($row_appartenir=$statement_appartenir->fetch()) 
                                            {
                                                //REQUETE afficher nom produit
                                                $id_produit=$row_appartenir['id_produit'];
                                                $query_produit="SELECT nom_produit  FROM produit INNER JOIN appartenir ON produit.id_produit = appartenir.id_produit ";
                                                ...
                                                //REQUETE afficher nom projet
                                                $id_projet=$row_appartenir['id_projet'];
                                                $query_projet="SELECT nom_projet  FROM projet INNER JOIN appartenir ON projet.id_projet = appartenir.id_projet ";
                                                ...
                                                $row_projet=$statement_projet->fetch();
                                                 ...
                                                 //REQUETE afficher tous les produits
                                                $query_commune="SELECT * FROM produit WHERE produit_id=:produit_id ";
                                                $statement_produit=$pdo->prepare($query_produit);
                                                $statement_produit->execute(array(':id_produit'=>$id_produit));
                                                $row_produit=$statement_produit->fetch();
                                                ...
                                                //REQUETE afficher tous les produits
                                                $query_ville="SELECT * FROM projet WHERE projet_id=:projet_id ";
                                                $statement_projet=$pdo->prepare($query_projet);
                                                $statement_projet->execute(array(':id_projet'=>$id_projet));
                                                $row_projet=$statement_projet->fetch();
                                                
                                                echo "<tr>";
                                                echo "<td ><strong>".$row_appartenir['id_projet']." ".$row_projet['nom_projet']."</strong></td>";
                                                echo "<td>".$row_produit['id_produit'].",".$row_produit['nom_produit'].",</td>";
                                                //echo "<td>".$row_maint['statut_contrat']."</td>";
    ...
    // requete pour afficher tous les noms produits si idproduit dans appartenir = idproduit dans produit

            $req=$pdo-> query('SELECT nom_produit  FROM produit INNER JOIN appartenir ON produit.id_produit = appartenir.id_produit ');

    // requete pour afficher tous les noms projets si idprojet dans appartenir = idprojet dans projet

            $req=$pdo-> query('SELECT nom_projet 
                    FROM projet 
                    INNER JOIN appartenir 
                    ON projet.id_projet = appartenir.id_projet ');

 
                            while ($row_projet=$statement_projet->fetch())
                            {
                                echo $row_projet['nom_projet'].'<br />';
        ...
        <!--<a class="btn btn-danger" data-toggle="modal"  href="delete/delete_utilisateur?">Supprimer</a>
        <a class="btn btn-info"  href="modifier/modifier_utilisateur">Modifier</a>-->

        <!-- Modal -->
       <!-- <div class="modal fade bg-theme11" id="" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog bg-theme11" role="document">
            <div class="modal-content">
              ...
              <div class="modal-body">
                  <p style="color: black">Vous allez supprimer cet utilisateur. <br> 
                  ce genre d'actions sont IRRÉVERSIBLES. <br> Vous pouvez maintenant annuller la suppression <br> 
                  si ce n'est pas l'action désirée </p>
              </div>
              <div class="modal-footer">
                <button type="button" class="btn btn-success" data-dismiss="modal">Annuler</button>
                <a type="button" class="btn btn-danger">Confirmer la suppression</a>
              </div>
            </div>
          </div>
        </div>
...
-->
  ...
  <!--start overlay-->
   ...
   <!--Start Back To Top Button-->
    </div> <a href="javaScript:void();" class="back-to-top"><i class="fa fa-angle-double-up"></i> </a> </div>
